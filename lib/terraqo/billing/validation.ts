...
export const checkoutSchema=z.object({
  ...
  customer:z.object({firstName:z.string().min(2).max(50).regex(/^[\p{L} ]+$/u),lastName:z.string().min(2).max(50).regex(/^[\p{L} ]+$/u),address:z.string().min(5).max(100),city:z.string().min(2).max(30),country:z.literal("PE"),phone:z.string().regex(/^\+?[0-9]{5,15}$/)}).strict(),
