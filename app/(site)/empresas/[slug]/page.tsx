...
export default async function PublicCompanyProfilePage({ params }: PageProps) {
  ...
  return (
    <main className="min-h-screen bg-[#f7faf8] text-[#082230]">
      <header className="border-b border-[#dbe8e5] bg-white/92 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-5 px-5 py-5">
          <Link href="/" className="flex items-center gap-3">
            <span className="grid h-11 w-11 place-items-center rounded-xl bg-[#06464c] p-2 text-xs font-black text-[#83efe2]">
            ...
            <span>
              ...
              <small className="block text-[10px] font-black uppercase tracking-[0.18em] text-primary">Perfil empresa</small>
            </span>
          </Link>
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-5 py-12 lg:grid-cols-[minmax(0,1fr)_380px] lg:py-16">
        <div className="rounded-[28px] border border-[#dbe8e5] bg-white p-8 shadow-[0_28px_90px_-48px_rgba(3,52,59,0.45)] md:p-12">
          <p className="text-xs font-black uppercase tracking-[0.22em] text-primary">{workspace.industry || "Empresa Terraqo"}</p>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {domainHref ? <Link href={domainHref} target="_blank" className="inline-flex h-11 items-center gap-2 rounded-md bg-primary px-5 text-sm font-black text-white">Visitar sitio <ArrowRight className="h-4 w-4" /></Link> : null}
            <Link href="/cuenta" className="inline-flex h-11 items-center gap-2 rounded-md border border-[#bddbd7] bg-white px-5 text-sm font-black text-[#06464c]">Entrar al portal</Link>
          </div>
        </div>

        <aside className="space-y-5">
          <div className="rounded-[24px] border border-[#dbe8e5] bg-[#061722] p-6 text-white shadow-[0_28px_90px_-48px_rgba(3,52,59,0.55)]">
            <p className="text-xs font-black uppercase tracking-[0.18em] text-[#83efe2]">Operación viva</p>
            <div className="mt-6 grid gap-3">
              <Metric icon={FolderKanban} value={workspace._count.projects} label="Proyectos registrados" />
              <Metric icon={BriefcaseBusiness} value={workspace._count.jobPosts} label="Oportunidades" />
              <Metric icon={UsersRound} value={workspace._count.professionalAffiliations} label="Profesionales vinculados" />
            </div>
          </div>
          <div id="contacto" className="rounded-[24px] border border-[#dbe8e5] bg-white p-6">
            ...
            <div className="mt-5 space-y-3 text-sm text-[#425e67]">
              {profile.coverage || workspace.locationCity || workspace.region ? <Info icon={MapPin}>{profile.coverage || [workspace.locationCity, workspace.region, workspace.country].filter(Boolean).join(", ")}</Info> : null}
              {profile.contactEmail ? <Info icon={Mail}>{profile.contactEmail}</Info> : null}
              {profile.contactPhone ? <Info icon={Phone}>{profile.contactPhone}</Info> : null}
              {domainHref ? <Info icon={Globe2}>{workspace.domain}</Info> : null}
            </div>
          </div>
        </aside>
      </section>
      ...
      <section className="mx-auto grid max-w-7xl gap-6 px-5 pb-16 lg:grid-cols-2">
        <Block id="servicios" title="Servicios principales" items={services} />
        <Block id="diferenciales" title="Diferenciales" items={differentiators} />
      </section>
    </main>
  );
}
...
function Metric({ icon: Icon, value, label }: { icon: typeof FolderKanban; value: number; label: string }) {
  return <div className="rounded-2xl border border-white/12 bg-white/8 p-4"><Icon className="h-5 w-5 text-[#83efe2]" /><strong className="mt-3 block font-display text-3xl">{value}</strong><span className="text-sm text-white/62">{label}</span></div>;
}

function Info({ icon: Icon, children }: { icon: typeof Mail; children: React.ReactNode }) {
  return <p className="flex items-start gap-3"><Icon className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><span>{children}</span></p>;
}
