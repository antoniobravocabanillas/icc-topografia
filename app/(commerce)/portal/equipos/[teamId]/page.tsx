...
export default async function ProfessionalTeamDetailPage({
  params,
}: {
  ...
  return (
    <div className="min-w-0 space-y-8 py-6 lg:py-8">
      ...
      <PortalPageHeading
        eyebrow={`${team.workspace.name} / Equipo privado`}
        title={team.name}
        description={team.purpose}
        action={
          isActive && team.conversationId ? (
            <Button asChild>
              <Link
                href={`/portal/mensajes?conversation=${team.conversationId}`}
              >
                <MessageSquareText className="mr-2 h-4 w-4" /> Abrir sala
              </Link>
            </Button>
          ) : undefined
        }
        ...
        <section className="grid gap-6 rounded-lg border border-amber-200 bg-amber-50 p-6 lg:grid-cols-[1fr_auto] lg:items-center">
          <div>
            ...
            <div className="mt-2 flex items-center gap-3">
              <UserAvatar
                name={team.owner.name || team.owner.email}
                image={team.owner.image}
                size="md"
              />
              <h2 className="text-xl font-bold">
                {team.owner.name || team.owner.email} te invito a colaborar
              </h2>
            </div>
            <p className="mt-2 text-sm leading-6 text-amber-900/70">
              Al aceptar entraras a la sala grupal y podras participar en
              mensajes y reuniones del equipo.
            </p>
          </div>
      ...
      <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
        <div className="rounded-lg border bg-white p-6 shadow-technical">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold">Personas del equipo</h2>
          </div>
          <div className="mt-6 divide-y">
            {team.members
              .filter(
                (member) =>
                  member.status !== "DECLINED" && member.status !== "REMOVED",
              )
              .map((member) => (
                <div
                  key={member.id}
                ...
                >
                  ...
                  <span
                    className={`w-fit rounded-md px-2 py-1 text-[11px] font-bold uppercase ${member.status === "ACTIVE" ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"}`}
                  >
                    {member.role === "OWNER"
                      ? "Responsable"
        ...
        <aside className="space-y-4">
          <div className="rounded-lg border bg-[#062f34] p-6 text-white shadow-technical">
            <div className="flex items-center gap-3">
              <UserAvatar
                name={team.owner.name || team.owner.email}
                image={team.owner.image}
              ...
              />
              <UserRoundCheck className="h-6 w-6 text-[#5de0d5]" />
            </div>
            <p className="mt-5 text-xs font-bold uppercase tracking-[0.18em] text-[#5de0d5]">
              Responsable
            </p>
          ...
          {team.project ? (
            <div className="rounded-lg border bg-white p-6">
              ...
              <p className="mt-2 font-semibold">{team.project.title}</p>
            </div>
          ) : null}
          <div className="rounded-lg border bg-white p-6">
            <Video className="h-5 w-5 text-primary" />
            <p className="mt-4 font-semibold">Mensajes y Terraqo Meet</p>
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              Cuando aceptas la invitacion, la sala del equipo aparece en
              Mensajes. Desde alli pueden iniciar una videollamada.
            </p>
          </div>
        </aside>
      </section>
