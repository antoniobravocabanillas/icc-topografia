...
import { builderLevel, ensureBuilders, redeemBuilderReward, submitBuilderContribution, terraqoTrustLevel } from "@/lib/terraqo/builders";
...
export default async function BuildersPage({ searchParams }: { searchParams: Promise<{ category?: string; sent?: string; redeemed?: string; error?: string }> }) {
  ...
  const [worklogCount, validatedWorklogCount, evidencedWorklogCount] = await Promise.all([
    prisma.terraqoWorklogEntry.count({ where: { authorId: session.user.id, deletedAt: null } }),
    prisma.terraqoWorklogEntry.count({ where: { authorId: session.user.id, deletedAt: null, evidenceStatus: { in: ["CONFIRMED", "VERIFIED"] } } }),
    prisma.terraqoWorklogEntry.count({ where: { authorId: session.user.id, deletedAt: null, OR: [{ media: { some: {} } }, { evidenceUrls: { isEmpty: false } }] } })
  ]);
  ...
  return <div className="min-w-0 pb-16 pt-6 lg:pt-9">
    ...
    <section className="mt-8"><SectionTitle eyebrow="Reputación verificable" title="Misiones de trabajo real" subtitle="No premian volumen: cada misión mejora la calidad, trazabilidad o validación de tu perfil." /><div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4"><Mission title="Primera bitácora" progress={Math.min(worklogCount, 1)} goal={1} href="/portal/bitacora" /><Mission title="Agregar evidencia" progress={Math.min(evidencedWorklogCount, 1)} goal={1} href="/portal/bitacora" /><Mission title="Obtener una validación" progress={Math.min(validatedWorklogCount, 1)} goal={1} href="/portal/validaciones" /><Mission title="Documentar trabajo consistente" progress={Math.min(worklogCount, 3)} goal={3} href="/portal/bitacora" /></div></section>
...
function Mission({ title, progress, goal, href }: { title: string; progress: number; goal: number; href: string }) { const complete = progress >= goal; return <Link href={href} className="rounded-2xl border border-[#d8e0ec] bg-white p-5 transition hover:-translate-y-0.5 hover:border-[#4374ba]/50"><div className="flex items-center justify-between"><span className={`grid h-10 w-10 place-items-center rounded-xl ${complete ? "bg-emerald-50 text-emerald-700" : "bg-[#edf4fd] text-[#4374ba]"}`}>{complete ? <CheckCircle2 className="h-5 w-5" /> : <Rocket className="h-5 w-5" />}</span><strong className="text-xs text-[#607083]">{progress}/{goal}</strong></div><h3 className="mt-4 font-display text-lg font-black text-[#0e1a26]">{title}</h3><div className="mt-4 h-1.5 overflow-hidden rounded-full bg-[#e5ebf2]"><div className="h-full rounded-full bg-[linear-gradient(90deg,#4374ba,#25c0d5)]" style={{ width: `${Math.min(100, (progress / goal) * 100)}%` }} /></div><p className="mt-3 text-xs font-bold text-[#4374ba]">{complete ? "Completada" : "Continuar misión"}</p></Link>; }
