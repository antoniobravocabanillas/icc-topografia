...
export default async function WorklogPage({ searchParams }: WorklogPageProps) {
  ...
  const enabledMemberships = memberships.filter((membership) =>
    workspaceHasModules(membership.workspace, [
      "PROFESSIONAL_NETWORK",
      "LIVE_CV",
    ]),
  );
  const workspaceIds = enabledMemberships.map(
  ...
  const days = Array.from(
    new Set(worklogs.map((worklog) => worklogDay(worklog.occurredAt))),
  );
  const visibleWorklogs = selectedDate
    ? worklogs.filter(
        (worklog) => worklogDay(worklog.occurredAt) === selectedDate,
      )
    : worklogs;

  const workspaceOptions = [
    { id: "", name: "Perfil personal Terraqo", projects: [] },
    ...enabledMemberships.map((membership) => ({
      id: membership.workspaceId,
      name: membership.workspace.brandName || membership.workspace.name,
      projects: projects
        .filter(
          (project) => project.terraqoWorkspaceId === membership.workspaceId,
        )
  ...
  return (
    <div className="min-w-0 space-y-5 py-4 sm:space-y-8 sm:py-6 lg:py-8">
      <PortalPageHeading
        ...
        description="Convierte avances, decisiones y entregables en una trayectoria verificable. Cada entrada conserva su contexto, proyecto y nivel de visibilidad."
      />

      <WorklogComposer
        workspaces={workspaceOptions}
        previousWorklogs={worklogs.map((worklog) => ({
          id: worklog.id,
          title: worklog.title,
      ...
      <section className="rounded-2xl border bg-white p-4 shadow-technical sm:rounded-lg sm:p-5">
        <div className="flex flex-col justify-between gap-3 lg:flex-row lg:items-end">
          <div>
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Selecciona un dia para revisar exactamente que evidencia fue
              generada.
            </p>
          </div>
          <Link
            href="/portal/bitacora"
            className={`min-h-11 rounded-xl border px-4 py-2.5 text-center text-sm font-semibold sm:min-h-0 sm:rounded-md sm:py-2 ${!selectedDate ? "bg-primary text-white" : "hover:bg-muted"}`}
          >
            Todas
          </Link>
        </div>
        <div className="mt-5 flex gap-2 overflow-x-auto pb-1">
          {days.map((day) => (
            ...
            >
              <span className="block font-mono text-[11px] uppercase tracking-[0.12em] opacity-70">
                {new Intl.DateTimeFormat("es-PE", { weekday: "short" }).format(
          ...
          {!days.length ? (
            <p className="text-sm text-muted-foreground">
              Tu calendario se activara con la primera bitacora.
            </p>
          ) : null}
        </div>
      </section>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        {visibleWorklogs.map((worklog) => (
          <WorklogCard
            ...
            continuityOptions={worklogs.map((candidate) => ({
              id: candidate.id,
