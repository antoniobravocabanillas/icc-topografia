...
const memberRoles: TerraqoMemberRole[] = [
  "OWNER",
  "ADMIN",
  "MANAGER",
  "MEMBER",
...
const planTiers: TerraqoPlanTier[] = [
  "FREE",
  "BASIC",
  "PROFESSIONAL",
  "PREMIUM",
...
async function createUserAction(formData: FormData) {
  ...
  if (!email || !name || password.length < 12 || !globalRoles.includes(role))
    throw new Error(
      "Completa nombre, correo, rol y una clave de al menos 12 caracteres.",
    );
...
async function updateUserAction(formData: FormData) {
  ...
  if (userId === session.user.id && role !== "SUPER_ADMIN")
    throw new Error(
      "No puedes retirar tu propio acceso de superadministrador.",
    );
  ...
  if (target?.role === "SUPER_ADMIN" && role !== "SUPER_ADMIN") {
    ...
    if (superAdmins <= 1)
      throw new Error("Terraqo debe conservar al menos un superadministrador.");
  }
...
async function resetPasswordAction(formData: FormData) {
  ...
  if (!userId || password.length < 12)
    throw new Error("La nueva clave debe tener al menos 12 caracteres.");
  await prisma.user.update({
    where: { id: userId },
...
async function upsertMembershipAction(formData: FormData) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const [profile, workspace] = await Promise.all([
      tx.terraqoProfessionalProfile.findUnique({
        where: { userId },
        select: { id: true, headline: true },
      }),
      tx.terraqoWorkspace.findUnique({
        where: { id: workspaceId },
        select: { name: true, brandName: true, companyId: true },
      }),
    ]);
    if (!profile || !workspace) return;
    await tx.terraqoProfessionalAffiliation.upsert({
      where: {
        professionalProfileId_workspaceId: {
...
async function updateMembershipAction(formData: FormData) {
  ...
  await prisma.terraqoWorkspaceMember.update({
...
async function sendProfileNudgeAction(formData: FormData) {
  ...
  const recipients = await prisma.user.findMany({
    ...
    select: {
      ...
      terraqoProfessionalProfile: {
        select: { username: true, onboardingSource: true },
      },
    },
  ...
  const sourceSlugs = [
    ...new Set(
      recipients
        .map(
          (recipient) =>
            recipient.terraqoProfessionalProfile?.onboardingSource?.match(
              /^public-careers:(.+)$/,
            )?.[1],
        )
  ...
  const sourceWorkspaces = sourceSlugs.length
    ? await prisma.terraqoWorkspace.findMany({
  ...
  for (const recipient of recipients) {
    ...
    const registrationSourceName = sourceSlug
      ? sourceNames.get(sourceSlug) || sourceSlug
      : null;
    let actionUrl = `${terraqoDomains.portal}/portal/configuracion`;
    if (!recipient.emailVerified) {
      const verification = await createEmailVerificationLinkToken(
        prisma,
        recipient.email,
      );
    ...
    const content = await renderProfileCompletionEmail({
      recipientName: recipient.name,
      profileUrl: actionUrl,
      customMessage,
      emailVerified: Boolean(recipient.emailVerified),
      hasUsername: Boolean(recipient.terraqoProfessionalProfile?.username),
      registrationSourceName,
    });
    const result = await sendTransactionalEmail({
      to: recipient.email,
      subject: "Completa tu perfil y aumenta tus oportunidades en Terraqo",
      ...content,
      tags: [{ name: "category", value: "profile-completion" }],
    });
    ...
    <section className="space-y-8">
      <header className="max-w-4xl">
        ...
        <p className="mt-3 text-muted-foreground">
          Control global de identidades Terraqo. El rol global define la
          superficie disponible y la membresia limita cada usuario a los datos
          de su workspace.
        </p>
      </header>

      {params?.status === "workspace-assigned" ? (
        <div className="rounded-md border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-800">
          Workspace asignado correctamente. Si el usuario es profesional, su
          vínculo empresarial también quedó validado.
        </div>
      ) : null}
      {params?.status === "emails-sent" ? (
        <div className="rounded-md border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-800">
          Se enviaron {params.sent || "0"} correos de acompañamiento.
        </div>
      ) : null}
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          [totals[0], "Usuarios"],
          [totals[1], "Correos verificados"],
          [totals[2], "Correos pendientes"],
          [totals[4], "Perfiles profesionales"],
        ].map(([value, label]) => (
      ...
      <Card>
        <CardHeader>
          <CardTitle>Seguimiento de activación y empleabilidad</CardTitle>
          <CardDescription>
            Filtra usuarios y envía una invitación personalizada para verificar
            el correo, crear su nombre de usuario, completar el perfil y
            registrar bitácoras.
          </CardDescription>
        </CardHeader>
          ...
          >
            ...
            <label className="grid gap-2 text-sm font-semibold">
              ...
              <Textarea
                name="customMessage"
                maxLength={1200}
                placeholder="Ej. Queremos ayudarte a fortalecer tu presencia profesional y mostrar mejor tu experiencia."
                className="min-h-24 bg-white font-normal"
              />
            </label>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-xs leading-5 text-muted-foreground">
                Se enviará a un máximo de 100 profesionales según el filtro de
                verificación seleccionado. Los pendientes recibirán un enlace
                nuevo de verificación.
              </p>
              <Button type="submit">Enviar a los usuarios filtrados</Button>
            </div>
          </form>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Alta manual para administradores, equipo Terraqo o cuentas
            asistidas.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form
            ...
            className="grid gap-3 lg:grid-cols-[1fr_1fr_220px_1fr_auto]"
          >
            ...
            <Input
              ...
              type="password"
              minLength={12}
              placeholder="Clave temporal (12+ caracteres)"
              required
            />
          ...
          <Card key={user.id}>
            <CardHeader className="gap-3 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div>
                  ...
                  {user.terraqoProfessionalProfile ? (
                    <p className="mt-2 text-sm text-muted-foreground">
                      Profesional:{" "}
                      {user.terraqoProfessionalProfile.headline ||
                      ...
                      | Plan {user.terraqoProfessionalProfile.planTier}
                    </p>
                  ) : null}
                </div>
              </div>
              <form
                ...
                className="grid gap-2 sm:grid-cols-[220px_200px_auto]"
              >
            ...
            <CardContent className="grid gap-5 xl:grid-cols-[1fr_380px]">
              <div>
                {user.terraqoProfessionalProfile ? (
                  <>
                    <div className="mb-4 grid gap-3 rounded-xl border bg-[#0e1a26] p-4 text-white sm:grid-cols-[1fr_auto] sm:items-center">
                      <div>
                        <p className="text-xs font-bold uppercase tracking-[0.14em] text-[#25c0d5]">
                          Vista previa del CV público
                        </p>
                        ...
                        <p className="mt-1 text-sm text-white/70">
                          {user.terraqoProfessionalProfile.headline ||
                            "Título profesional por completar"}
                        </p>
                        <p className="mt-2 text-xs text-white/50">
                          {user.terraqoProfessionalProfile.username
                            ? `terraqoglobal.com/cv/${user.terraqoProfessionalProfile.username}`
                            : "Aún no creó su nombre de usuario"}
                        </p>
                      </div>
                      {user.terraqoProfessionalProfile.username ? (
                        <Button
                        ...
                        >
                          <a
                            href={`${terraqoDomains.public}/cv/${user.terraqoProfessionalProfile.username}`}
                            target="_blank"
                          >
                            Ver perfil
                          </a>
                    ...
                    <form
                      action={sendProfileNudgeAction}
                      className="mb-4 flex flex-col gap-2 rounded-md border p-3 sm:flex-row sm:items-end"
                    >
                      <input type="hidden" name="userId" value={user.id} />
                      <label className="grid flex-1 gap-1 text-xs font-bold">
                        ...
                        <Input
                          name="customMessage"
                          placeholder="Mensaje opcional para este usuario"
                    ...
                    <form
                      action={updateProfessionalPlanAction}
                    ...
                    >
                      <input
                        type="hidden"
                        name="profileId"
                        value={user.terraqoProfessionalProfile.id}
                      />
                      <div>
                        <p className="font-semibold">
                          Plan profesional Terraqo
                        </p>
                        <p className="text-xs text-muted-foreground">
                          Pertenece al perfil personal y no al workspace de una
                          empresa.
                        </p>
                      </div>
                      <select
                        name="planTier"
                        defaultValue={user.terraqoProfessionalProfile.planTier}
                        className="h-10 rounded-md border bg-background px-3 text-sm"
                      >
                        {planTiers.map((tier) => (
                ...
                <div className="space-y-2">
                  {user.terraqoMemberships.map((membership) => (
                    <form
                      ...
                      className="grid gap-2 rounded-md border bg-muted/20 p-3 sm:grid-cols-[1fr_180px_130px_auto] sm:items-center"
                    >
                      <input
                        type="hidden"
                        name="membershipId"
                  ...
                  {!user.terraqoMemberships.length ? (
                ...
                <form
                  action={upsertMembershipAction}
                  className="mt-3 grid gap-2 rounded-md border border-dashed p-3 sm:grid-cols-[1fr_180px_auto]"
                >
                  <input type="hidden" name="userId" value={user.id} />
                  <select
                    name="workspaceId"
              ...
              <form
                ...
                className="self-start rounded-md border bg-muted/20 p-4"
              >
                <input type="hidden" name="userId" value={user.id} />
                <p className="font-semibold">Restablecer acceso</p>
                <p className="mt-1 text-xs text-muted-foreground">
                  La clave se cifra antes de guardarse y nunca vuelve a
                  mostrarse.
                </p>
                <Input
                  name="password"
