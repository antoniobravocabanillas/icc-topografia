...
import { moderateWorklogReputation, reviewBuilderContribution } from "@/lib/terraqo/builders";
...
async function moderateWorklogAction(formData: FormData) {
  "use server";
  await requireAdminPage(["SUPER_ADMIN"]);
  await moderateWorklogReputation({ worklogId: String(formData.get("worklogId") || ""), status: String(formData.get("status") || "OBSERVED") as "OBSERVED" | "REJECTED" | "DECLARED", note: String(formData.get("note") || "") });
  revalidatePath("/admin/terraqo/builders");
...
export default async function BuildersAdminPage() {
  await requireAdminPage(["SUPER_ADMIN"]);
  const [contributions, accounts, rewards, redeemed, leadingBuilders, worklogMetrics, riskyWorklogs] = await Promise.all([
    ...
    Promise.all([
      ...
      prisma.terraqoWorklogEntry.count({ where: { deletedAt: null, evidenceStatus: { in: ["CONFIRMED", "VERIFIED"] } } }),
      prisma.terraqoWorklogEntry.count({ where: { deletedAt: null, moderationStatus: { in: ["DUPLICATE", "OBSERVED", "REJECTED"] } } }),
      prisma.terraqoBuilderAccount.aggregate({ _sum: { pendingPoints: true, trustScore: true } })
    ]),
    prisma.terraqoWorklogEntry.findMany({ where: { deletedAt: null, OR: [{ moderationStatus: { in: ["DUPLICATE", "OBSERVED"] } }, { qualityScore: { lt: 35 } }] }, include: { author: { select: { name: true, email: true } }, workspace: { select: { name: true, brandName: true } } }, orderBy: { createdAt: "desc" }, take: 40 })
  ]);
  ...
  return <section className="space-y-7"><header><p className="font-mono text-xs font-bold uppercase tracking-[0.16em] text-primary">Plataforma Terraqo</p><h1 className="mt-2 font-display text-3xl font-black">Terraqo Builders</h1><p className="mt-2 text-sm text-muted-foreground">Revisa aportes, acredita TQ y controla el programa sin asignaciones automáticas por reportes enviados.</p></header>
    ...
    <div className="grid gap-4 md:grid-cols-4"><Metric icon={<BadgeCheck />} label="Bitácoras creadas" value={worklogMetrics[0]} /><Metric icon={<Trophy />} label="Bitácoras validadas" value={worklogMetrics[1]} /><Metric icon={<Clock3 />} label="Señales de riesgo" value={worklogMetrics[2]} /><Metric icon={<Coins />} label="TQ pendientes" value={worklogMetrics[3]._sum.pendingPoints || 0} /></div>
    <div className="overflow-hidden rounded-xl border bg-white"><div className="border-b p-5"><h2 className="font-display text-xl font-black">Control de trabajo documentado</h2><p className="mt-1 text-sm text-muted-foreground">Revisa duplicados y registros con baja calidad antes de que sus TQ sean liberados.</p></div><div className="divide-y">{riskyWorklogs.map((worklog) => <article key={worklog.id} className="grid gap-4 p-5 lg:grid-cols-[1fr_auto]"><div><div className="flex flex-wrap gap-2"><span className="rounded-full bg-amber-100 px-2.5 py-1 text-[10px] font-bold text-amber-900">{worklog.moderationStatus}</span><span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-bold text-slate-700">Calidad {worklog.qualityScore}%</span></div><h3 className="mt-3 font-display text-lg font-black">{worklog.title}</h3><p className="mt-1 text-xs text-muted-foreground">{worklog.author.name || worklog.author.email} · {worklog.workspace?.brandName || worklog.workspace?.name || "Personal"}</p><p className="mt-3 text-sm leading-6 text-muted-foreground">{worklog.summary}</p></div><form action={moderateWorklogAction} className="grid min-w-[280px] gap-2 rounded-lg border bg-muted/20 p-3"><input type="hidden" name="worklogId" value={worklog.id} /><select name="status" defaultValue="OBSERVED" className="h-10 rounded-md border bg-white px-3 text-sm"><option value="OBSERVED">Observar</option><option value="REJECTED">Rechazar</option><option value="DECLARED">Liberar para recalcular</option></select><textarea name="note" rows={2} className="rounded-md border bg-white p-2 text-sm" placeholder="Motivo o corrección solicitada" /><button className="h-10 rounded-md bg-primary px-4 text-sm font-bold text-white">Aplicar revisión</button></form></article>)}{!riskyWorklogs.length ? <p className="p-8 text-center text-sm text-muted-foreground">No hay señales de riesgo pendientes.</p> : null}</div></div>
