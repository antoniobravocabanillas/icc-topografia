...
export function PrivateNotesManager({ workspaces, apiBase = "/api/terraqo" }: { workspaces: WorkspaceOption[]; apiBase?: string }) {
  ...
  return (
    <div className="space-y-7">
      <section className="overflow-hidden rounded-lg border border-[#d8e0ec] bg-white shadow-[0_20px_60px_rgba(9,50,58,0.07)]">
        <div className="grid gap-0 xl:grid-cols-[0.8fr_1.2fr]">
          <div className="bg-[#0e1a26] p-7 text-white lg:p-10">
            <ShieldCheck className="h-8 w-8 text-[#25c0d5]" />
            <p className="mt-8 font-mono text-xs font-bold uppercase tracking-[0.18em] text-[#25c0d5]">Espacio personal</p>
          ...
          <form onSubmit={submit} className="grid gap-4 p-6 lg:p-10">
            ...
            <div className="grid grid-cols-2 gap-2 rounded-lg bg-[#e8eef7] p-1.5">
              <button type="button" onClick={() => setKind("SIMPLE")} className={`min-h-11 rounded-md text-sm font-semibold ${kind === "SIMPLE" ? "bg-white text-[#0e1a26] shadow-sm" : "text-[#527078]"}`}><StickyNote className="mr-2 inline h-4 w-4" />Simple</button>
              <button type="button" onClick={() => setKind("SECURE")} className={`min-h-11 rounded-md text-sm font-semibold ${kind === "SECURE" ? "bg-[#0e1a26] text-white shadow-sm" : "text-[#527078]"}`}><LockKeyhole className="mr-2 inline h-4 w-4" />Segura</button>
            </div>
            <input value={title} onChange={(event) => setTitle(event.target.value)} maxLength={140} required placeholder="Título" className="h-12 rounded-lg border border-[#d8e0ec] px-4 outline-none focus:border-[#4374ba]" />
            <textarea value={body} onChange={(event) => setBody(event.target.value)} maxLength={24000} required placeholder="Escribe tu nota..." className="min-h-40 resize-y rounded-lg border border-[#d8e0ec] p-4 outline-none focus:border-[#4374ba]" />
            {kind === "SECURE" ? <label className="grid gap-2 text-sm font-semibold">Frase de acceso de la bóveda<input type="password" value={passphrase} onChange={(event) => setPassphrase(event.target.value)} minLength={12} required placeholder="Mínimo 12 caracteres" autoComplete="new-password" className="h-12 rounded-lg border border-[#d8e0ec] px-4 font-normal outline-none focus:border-[#4374ba]" /></label> : null}
            <button disabled={busy || !workspaceId} className="min-h-12 rounded-lg bg-[#4374ba] px-5 font-semibold text-white transition hover:bg-[#4374ba] disabled:opacity-50"><Plus className="mr-2 inline h-4 w-4" />{busy ? "Protegiendo..." : "Guardar nota"}</button>
          </form>
        </div>
      </section>
      ...
      <section>
        <div className="mb-4 flex items-end justify-between gap-4"><div><p className="font-mono text-xs font-bold uppercase tracking-[0.16em] text-[#4374ba]">Tu archivo personal</p><h2 className="mt-2 font-display text-2xl font-bold">Notas guardadas</h2></div><span className="font-mono text-xs text-[#607083]">{notes.length} notas</span></div>
        <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">
          {notes.map((note) => {
            const content = note.kind === "SIMPLE" ? { title: note.title || "Sin título", body: note.body || "" } : unlocked[note.id];
            return <article key={note.id} className={`min-h-64 rounded-lg border p-6 ${note.kind === "SECURE" ? "border-[#98d3cd] bg-[#0e1a26] text-white" : "border-[#d8e0ec] bg-white"}`}>
              <div className="flex items-start justify-between gap-4"><span className={`grid h-10 w-10 place-items-center rounded-lg ${note.kind === "SECURE" ? "bg-white/10 text-[#66e3d8]" : "bg-[#e8f7f5] text-[#4374ba]"}`}>{note.kind === "SECURE" ? <KeyRound className="h-5 w-5" /> : <StickyNote className="h-5 w-5" />}</span><button onClick={() => remove(note.id)} className="grid h-9 w-9 place-items-center rounded-lg text-current/55 hover:bg-black/5 hover:text-red-600" aria-label="Eliminar nota"><Trash2 className="h-4 w-4" /></button></div>
