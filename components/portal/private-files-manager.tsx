...
export function PrivateFilesManager({ workspaces, userId, apiBase = "/api/terraqo" }: { workspaces: WorkspaceOption[]; userId?: string; apiBase?: string }) {
  ...
  return <div className="space-y-7">
    <section className="rounded-lg border border-[#d8e0ec] bg-white p-6 shadow-[0_20px_60px_rgba(9,50,58,0.07)] lg:p-9">
      <div className="grid gap-8 xl:grid-cols-[0.72fr_1.28fr]">
        <div>
          <span className="grid h-12 w-12 place-items-center rounded-lg bg-[#e8f7f5] text-[#4374ba]"><FolderOpen className="h-6 w-6" /></span>
          <p className="mt-6 font-mono text-xs font-bold uppercase tracking-[0.17em] text-[#4374ba]">Repositorio de trabajo</p>
          ...
          <div className="mt-6 flex items-start gap-3 rounded-lg bg-[#e8eef7] p-4 text-sm text-[#466069]"><LockKeyhole className="mt-0.5 h-5 w-5 shrink-0 text-[#4374ba]" /><p>Los archivos privados solo son visibles desde tu cuenta. Los compartidos pueden ser revisados por administradores autorizados del mismo workspace.</p></div>
        </div>
        <form onSubmit={submit} className="grid gap-4 rounded-lg border border-[#d6e5e2] bg-[#f3f3f3] p-5 sm:grid-cols-2 lg:p-6">
          ...
          <label className="grid gap-2 text-sm font-semibold">Título<input name="title" required maxLength={160} placeholder="Ej. Plano de replanteo" className="h-12 rounded-lg border px-4 font-normal outline-none focus:border-[#4374ba]" /></label>
          ...
          <label className="grid gap-2 text-sm font-semibold">Proyecto o contexto <span className="font-normal text-[#789098]">(opcional)</span><input name="projectName" maxLength={160} placeholder="Proyecto, cliente o actividad" className="h-12 rounded-lg border px-4 font-normal outline-none focus:border-[#4374ba]" /></label>
          ...
          <label className="grid gap-2 text-sm font-semibold sm:col-span-2">Descripción <span className="font-normal text-[#789098]">(opcional)</span><textarea name="description" maxLength={1000} placeholder="Qué contiene y para qué se utiliza" className="min-h-24 rounded-lg border p-4 font-normal outline-none focus:border-[#4374ba]" /></label>
          ...
          <button disabled={busy || !workspaceId} className="min-h-12 rounded-lg bg-[#4374ba] px-5 font-semibold text-white transition hover:bg-[#4374ba] disabled:opacity-50 sm:col-span-2"><UploadCloud className="mr-2 inline h-4 w-4" />{busy ? "Cargando..." : "Guardar archivo"}</button>
        </form>
      </div>
    </section>
    ...
    <section>
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="font-mono text-xs font-bold uppercase tracking-[0.16em] text-[#4374ba]">Biblioteca privada</p><h2 className="mt-2 font-display text-2xl font-bold">Archivos guardados</h2></div><select value={filter} onChange={(event) => setFilter(event.target.value)} className="h-11 rounded-lg border bg-white px-3 text-sm"><option value="ALL">Todas las categorías</option>{categories.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
      <div className="mt-5 overflow-hidden rounded-lg border border-[#d8e0ec] bg-white">
        {visible.map((file, index) => <article key={file.id} className={`grid gap-4 p-5 lg:grid-cols-[minmax(0,1fr)_190px_auto] lg:items-center ${index ? "border-t border-[#e1ebe9]" : ""}`}>
          <div className="flex min-w-0 items-start gap-4"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-lg bg-[#e8f7f5] text-[#4374ba]"><FileIcon contentType={file.contentType} fileName={file.fileName} /></span><div className="min-w-0"><h3 className="truncate font-semibold text-[#092731]">{file.title}</h3><p className="mt-1 truncate text-sm text-[#647c84]">{file.fileName} · {formatSize(file.size)}{file.projectName ? ` · ${file.projectName}` : ""}</p>{file.description ? <p className="mt-2 line-clamp-2 text-sm text-[#536b73]">{file.description}</p> : null}</div></div>
          <div className="flex items-center gap-2 text-xs font-semibold text-[#577078]">{file.visibility === "PRIVATE" ? <><LockKeyhole className="h-4 w-4 text-[#4374ba]" />Solo tú</> : <><Share2 className="h-4 w-4 text-[#4374ba]" />Empresa vinculada</>}{file.userId !== userId && file.user ? <span className="ml-2 truncate font-normal">· {file.user.name || file.user.email}</span> : null}</div>
          <div className="flex flex-wrap gap-2 lg:justify-end">{canPreview(file) ? <button onClick={() => setPreview(file)} className="grid h-10 w-10 place-items-center rounded-lg border text-[#31505a] hover:bg-[#e8eef7]" title="Previsualizar"><Eye className="h-4 w-4" /></button> : null}<a href={`${apiBase}/files/${file.id}`} className="grid h-10 w-10 place-items-center rounded-lg border text-[#31505a] hover:bg-[#e8eef7]" title="Descargar"><Download className="h-4 w-4" /></a>{file.userId === userId || !userId ? <button onClick={() => remove(file)} className="grid h-10 w-10 place-items-center rounded-lg border text-[#8d4b4b] hover:bg-red-50" title="Eliminar"><Trash2 className="h-4 w-4" /></button> : null}</div>
        </article>)}
