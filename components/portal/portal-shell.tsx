...
export function PortalShell({
  ...
  useEffect(() => {
    setSidebarCollapsed(
      window.localStorage.getItem("terraqo:portal-sidebar") === "collapsed",
    );
  }, []);
    ...
    const closeOutside = (event: PointerEvent) => {
      document
        .querySelectorAll<HTMLDetailsElement>(
          "details[data-portal-popover][open]",
        )
        .forEach((popover) => {
  ...
  return (
    <div
      className="min-h-screen overflow-x-hidden text-[#0e1a26]"
      style={{ backgroundColor: visualIdentity.backgroundColor }}
    >
      <header className="sticky top-0 z-50 border-b border-[#d8e0ec] bg-white/95 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1720px] items-center gap-4 px-4 sm:h-[76px] sm:px-6 xl:px-8">
          <Link
            ...
            aria-label="Ir al resumen del portal"
          >
            <span
              className="grid h-10 w-10 shrink-0 place-items-center overflow-hidden rounded-lg border bg-white p-1.5"
              style={{ borderColor: withAlpha(accentColor, "55") }}
            >
              <TerraqoLogo
                src={brandLogo}
                variant="mark"
            ...
            <span className="hidden min-w-0 sm:block">
              ...
              <small className="block truncate text-xs text-[#607083]">
                {portalLabel} · Plan {planLabel}
              </small>
            </span>
          </Link>
          ...
          {portalType === "professional" ? (
            <form
              ...
              method="get"
              role="search"
              className="mx-auto hidden min-w-0 max-w-2xl flex-1 md:block"
            >
              <label className="relative block">
                ...
                <input
                  name="q"
                  type="search"
                  placeholder="Buscar profesionales, empresas, equipos o habilidades..."
                  className="h-11 w-full rounded-full border border-[#d8e0ec] bg-[#f6f8fa] pl-11 pr-4 text-sm font-medium text-[#2f4154] outline-none transition focus:border-primary/45 focus:bg-white focus:ring-4 focus:ring-primary/10"
                />
              </label>
            </form>
          ) : null}
          ...
          <div className="ml-auto flex items-center gap-2 sm:gap-3 md:ml-0">
            ...
            <details data-portal-popover className="group relative">
              ...
              <div
                ...
                className="fixed inset-x-3 top-[72px] z-50 rounded-2xl border border-[#d8e0ec] bg-white p-3 shadow-[0_24px_65px_rgba(14,26,38,0.22)] sm:absolute sm:inset-x-auto sm:right-0 sm:top-[calc(100%+10px)] sm:w-72 sm:rounded-xl sm:p-2"
              >
                <div className="border-b border-[#e7ecf2] px-3 py-2.5">
                  <p className="truncate text-sm font-bold">
                    {name || "Cuenta Terraqo"}
                  </p>
                  <p className="mt-0.5 truncate text-xs text-[#607083]">
                    {headline || `${portalLabel} · Plan ${planLabel}`}
                  </p>
                </div>
                {portalType === "professional" ? (
                  <Link
                    href="/portal/perfil"
                    className="mt-1 flex h-10 items-center gap-3 rounded-lg px-3 text-sm font-semibold text-[#35485b] hover:bg-[#eef3f7]"
                  >
                ...
                <Link
                  ...
                  className="flex h-10 items-center gap-3 rounded-lg px-3 text-sm font-semibold text-[#35485b] hover:bg-[#eef3f7]"
                >
      ...
      <div className="mx-auto max-w-[1720px] px-4 sm:px-6 xl:px-8">
        <div className="sticky top-[76px] z-40 -mx-4 hidden border-b border-[#d8e0ec] bg-white/88 px-4 py-3 shadow-[0_14px_28px_rgba(15,59,67,0.06)] backdrop-blur-xl sm:-mx-6 sm:px-6 md:block xl:hidden">
          <div className="mb-2 flex items-center justify-between gap-4">
            <p className="font-mono text-[11px] font-bold uppercase tracking-[0.16em] text-[#245da7]">
              Navegación
            </p>
            <span className="truncate text-xs font-semibold text-[#607083]">
              {portalLabel}
            </span>
          </div>
        ...
        <div
          className={`grid min-h-[calc(100vh-64px)] gap-5 pb-24 transition-[grid-template-columns,gap] duration-300 sm:min-h-[calc(100vh-76px)] sm:gap-7 sm:pb-0 xl:gap-7 ${sidebarCollapsed ? "xl:grid-cols-[76px_minmax(0,1fr)]" : "xl:grid-cols-[250px_minmax(0,1fr)] xl:gap-9"}`}
        >
          <aside
          ...
          >
            <div className="sticky top-[108px]">
              ...
              >
                <p
                  className={
                    sidebarCollapsed
                      ? "sr-only"
                      : "font-mono text-[11px] font-bold uppercase tracking-[0.16em] text-[#245da7]"
                  }
                >
                  Navegación
                </p>
                <button
                  ...
                  className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-[#d8e0ec] bg-white text-[#35485b] shadow-sm transition hover:border-primary/30 hover:bg-primary/5 hover:text-primary"
                >
                  {sidebarCollapsed ? (
              ...
              <div className="mt-7 border-t border-[#d8e0ec] pt-6">
                <a
                  href="mailto:proyectos@icctopografia.com"
                  title={sidebarCollapsed ? "Ayuda y soporte" : undefined}
                  className={`flex min-h-11 items-center rounded-lg text-sm font-semibold text-[#35485b] hover:bg-white hover:text-primary ${sidebarCollapsed ? "justify-center px-2" : "gap-3 px-3.5 py-3"}`}
                >
                ...
                <SignOutButton
                  ...
                  className={`mt-2 w-full border-0 bg-transparent text-[#35485b] shadow-none hover:bg-white ${sidebarCollapsed ? "justify-center px-2" : "justify-start px-3.5"}`}
                />
              </div>
              <div
                className={`mt-8 rounded-lg bg-[#eaf6f4] text-sm text-[#35485b] ${sidebarCollapsed ? "grid place-items-center p-3" : "p-4"}`}
                title={
                  sidebarCollapsed
                    ? "Tu información está protegida por permisos de Terraqo."
                    : undefined
                }
              >
                <ShieldCheck
                ...
                <p
                  className={
                    sidebarCollapsed ? "sr-only" : "mt-3 font-semibold"
                  }
                >
                  Tu información está protegida por permisos de Terraqo.
                </p>
          ...
          <main className="min-w-0">{(!planTier||planTier==="FREE")&&<aside className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-blue-200 bg-white p-4 text-slate-900" aria-label="Mejorar mi plan"><div><p className="font-semibold">Tu cuenta gratuita es solo el comienzo.</p><p className="text-sm text-slate-600">Compara capacidades y elige cuándo ampliar tu plan.</p></div><Link href={pathname.endsWith("/membresia") ? "#planes" : "https://portal.terraqoglobal.com/membresia#planes"} className="rounded-lg bg-blue-700 px-4 py-2 font-semibold text-white hover:bg-blue-800">Explorar membresías</Link></aside>}{children}</main>
        </div>
      </div>

      <nav
        className="fixed inset-x-0 bottom-0 z-50 border-t border-[#d8e0ec] bg-white/95 px-[max(0.5rem,env(safe-area-inset-left))] pb-[max(0.45rem,env(safe-area-inset-bottom))] pt-1.5 shadow-[0_-12px_35px_rgba(14,26,38,0.12)] backdrop-blur-xl md:hidden"
          ...
          {mobilePrimaryItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                aria-current={active ? "page" : undefined}
                className="flex min-h-14 flex-col items-center justify-center gap-1 rounded-xl px-1 text-[10px] font-bold"
                style={
                  active
                    ? {
              ...
              >
                ...
                <span className="max-w-full truncate">
                  {item.label === "Experiencias" ? "Experiencia" : item.label}
                </span>
              </Link>
            );
          })}
          <details data-portal-popover className="group relative">
            <summary className="flex min-h-14 cursor-pointer list-none flex-col items-center justify-center gap-1 rounded-xl px-1 text-[10px] font-bold text-[#607083] [&::-webkit-details-marker]:hidden">
              <MoreHorizontal className="h-5 w-5" />
              <span>Más</span>
            </summary>
            <div
              onClick={closePopover}
              className="fixed inset-x-3 bottom-[calc(4.75rem+env(safe-area-inset-bottom))] grid max-h-[calc(100dvh-7rem)] grid-cols-2 gap-2 overflow-y-auto rounded-2xl border border-[#d8e0ec] bg-white p-3 shadow-[0_24px_70px_rgba(14,26,38,0.24)]"
            >
      ...
      {writingAssistantEnabled ? (
        <ClientFeatureBoundary feature="writing-assistant">
          <WorkspaceWritingAssistant />
        </ClientFeatureBoundary>
      ) : null}
      {portalType === "professional" ? (
        <MessageDrawer currentUserId={currentUserId} />
      ) : null}
    </div>
  );
}
