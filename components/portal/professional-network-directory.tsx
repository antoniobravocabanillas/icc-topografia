...
type StoryWorklog = { id: string; title: string; summary: string; outcome?: string | null; type: string; evidenceStatus: string; moderationStatus: string; tqPointsAwarded: number; trustScoreAwarded: number; skills: string[]; occurredAt: string; workspaceName?: string | null; projectName?: string | null; mediaIds: string[]; reactions: number; comments: number };
type ProfessionalItem = { id: string; name: string; image?: string | null; headline?: string | null; roleTitle?: string | null; city?: string | null; locationCity?: string | null; status: string; verified: boolean; workspaceName: string; skills: string[]; visibleExperiences: number; verifiedExperiences: number; lastSeenAt?: string | null; onlineUntil?: string | null; profileBoostUntil?: string | null; foundingBuilder?: boolean; latestWorklog?: StoryWorklog | null; updatedAt: string };
...
export function ProfessionalNetworkDirectory({ initialQuery = "", jobAds, professionals, companies, groups }: DirectoryProps) {
  ...
  const visibleProfessionals = useMemo(() => {
    ...
    return professionals.filter((item) => {
      ...
      const rightBoosted = Boolean(right.profileBoostUntil && Date.parse(right.profileBoostUntil) > Date.now());
      const leftBoosted = Boolean(left.profileBoostUntil && Date.parse(left.profileBoostUntil) > Date.now());
      return Number(rightBoosted) - Number(leftBoosted) || Number(right.verified) - Number(left.verified) || right.verifiedExperiences - left.verifiedExperiences || Date.parse(right.updatedAt) - Date.parse(left.updatedAt);
    });
  ...
  return <section className="relative isolate overflow-hidden rounded-[26px] border border-[#4374ba]/25 bg-[#07111f] px-5 py-7 text-white shadow-[0_28px_70px_rgba(14,26,38,0.18)] sm:px-8 lg:min-h-[390px] lg:px-10 lg:py-10">
    ...
    <div className="relative grid gap-8 lg:grid-cols-[minmax(0,1.2fr)_250px_310px] lg:items-center">
      ...
      <aside className="rounded-[20px] border border-[#488ac9]/25 bg-[#102338]/72 p-5 shadow-[0_20px_50px_rgba(0,0,0,0.18)] backdrop-blur-xl sm:p-6">
        ...
        <Link href="/portal/recompensas" className="mt-4 flex items-center justify-between gap-3 text-xs font-black text-[#80e8f4] transition hover:text-white">Conoce el sistema de recompensas<ArrowRight className="h-4 w-4" /></Link>
      </aside>
    </div>
  </section>;
}
...
function NetworkStories({ professionals, companies }: { professionals: ProfessionalItem[]; companies: CompanyItem[] }) {
  ...
  return <>
    <section aria-label="Trabajo documentado" className="mt-5 rounded-[22px] border border-[#d7e3e8] bg-white px-4 py-4 shadow-[0_10px_30px_rgba(11,35,48,0.045)] sm:px-6">
      <div className="flex items-center justify-between gap-4"><div><h2 className="font-display text-base font-bold text-[#102535]">Trabajo documentado</h2><p className="mt-0.5 text-[11px] text-[#78909c]">Evidencia técnica reciente de tu red</p></div><Link href="/portal/bitacora" className="text-[11px] font-bold text-[#087d79]">Registrar bitácora</Link></div>
      <div className="-mx-2 mt-4 flex gap-5 overflow-x-auto px-2 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <Link href="/portal/bitacora" className="group w-[74px] shrink-0 text-center"><span className="mx-auto grid h-16 w-16 place-items-center rounded-full border border-dashed border-[#9ebdc0] bg-[#f7fbfb] text-[#52717b] transition group-hover:border-[#087d79] group-hover:text-[#087d79]"><Plus className="h-5 w-5" /></span><span className="mt-2 block text-[11px] font-semibold text-[#526878]">Documentar</span></Link>
  ...
  return <div role="dialog" aria-modal="true" aria-label={`Historia de ${profile.name}`} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }} className={`fixed inset-0 z-[100] grid place-items-center bg-[#06151f]/70 p-3 backdrop-blur-sm transition-opacity duration-200 sm:p-6 ${closing ? "opacity-0" : "opacity-100"}`}>
    <article className={`relative grid max-h-[min(820px,calc(100dvh-24px))] w-full max-w-[920px] overflow-hidden rounded-[26px] border border-white/40 bg-white shadow-[0_36px_110px_rgba(0,0,0,0.34)] transition duration-200 md:grid-cols-[minmax(310px,1.05fr)_minmax(330px,0.95fr)] ${closing ? "translate-y-3 scale-[0.985] opacity-0" : "translate-y-0 scale-100 opacity-100"}`}>
      ...
      <div className="min-h-0 overflow-y-auto p-6 sm:p-8 md:pt-20">
        {worklog ? <>
          <div className="flex flex-wrap items-center gap-2"><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-[#087d79]">Última bitácora</p><span className="rounded-full bg-[#eaf7f6] px-2.5 py-1 text-[10px] font-bold text-[#087d79]">{worklog.moderationStatus === "STRONG_VERIFIED" ? "Validada con evidencia fuerte" : worklog.moderationStatus === "PRIVATE_VALIDATED" ? "Privada validada" : worklog.moderationStatus === "VALIDATED" ? "Validada" : worklog.moderationStatus === "COMPLETE" ? "Evidencia completa" : "Declarada"}</span></div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">{worklog.skills.slice(0, 4).map((skill) => <span key={skill} className="rounded-lg bg-[#eff4f5] px-2.5 py-1 text-[11px] font-bold text-[#526878]">#{skill}</span>)}</div><div className="mt-4 flex gap-2"><span className="rounded-lg bg-[#edf4fd] px-3 py-2 text-xs font-bold text-[#4374ba]">+{worklog.tqPointsAwarded} TQ en espera</span><span className="rounded-lg bg-[#eaf7f6] px-3 py-2 text-xs font-bold text-[#087d79]">+{worklog.trustScoreAwarded} confianza</span></div>
...
function ProfessionalCard({ profile, compact }: { profile: ProfessionalItem; compact: boolean }) {
  const available = ["AVAILABLE", "OPEN_TO_PROJECTS"].includes(profile.status); const online = Boolean(profile.onlineUntil && Date.parse(profile.onlineUntil) > Date.now()); const boosted = Boolean(profile.profileBoostUntil && Date.parse(profile.profileBoostUntil) > Date.now()); const title = profile.roleTitle || profile.headline || "Perfil profesional";
  return <article className={`group relative overflow-hidden rounded-[22px] border border-[#d8e4e8] bg-white p-5 shadow-[0_12px_38px_rgba(11,35,48,0.055)] transition duration-300 hover:-translate-y-1 hover:border-[#13a8a4]/50 hover:shadow-[0_20px_48px_rgba(8,125,121,0.12)] ${compact ? "lg:grid lg:grid-cols-[minmax(280px,1.15fr)_minmax(220px,1fr)_auto] lg:items-center lg:gap-6" : "flex min-h-[330px] flex-col"}`}>
    ...
    <div className={`${compact ? "mt-4 min-w-[220px] lg:mt-0" : "mt-5 border-t border-[#e4ecef] pt-4"}`}><div className="mb-3 flex items-center justify-between gap-2"><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${boosted ? "bg-[#edf4fd] text-[#4374ba]" : available ? "bg-[#e4f7ed] text-[#14764a]" : "bg-[#f1f4f5] text-[#6b7e8a]"}`}>{boosted ? "Perfil destacado" : available ? "Disponible para proyectos" : "No disponible"}</span><span className="text-[10px] font-semibold text-[#7a8d99]">{profile.foundingBuilder ? "Founding Builder" : online ? "En línea" : "Fuera de línea"}</span></div><div className="grid grid-cols-2 gap-2"><ButtonLink href={`/portal/profesionales/${profile.id}`}>Ver perfil</ButtonLink><ButtonLink href="/portal/mensajes" primary><MessageSquare className="h-3.5 w-3.5" />Conectar</ButtonLink></div></div>
  </article>;
}
